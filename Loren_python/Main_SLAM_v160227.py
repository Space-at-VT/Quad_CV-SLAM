...
for frameIdx in range(firstFrame,800):
    ...
    if not(np.mod(frameIdx,5)) and (frameIdx != firstFrame):
        ...
        newPtIdx = np.setdiff1d(nums,mTrainIdx)
    ...
    # increment point life tracker
    actPtLife[stIdx[:,0]==1] += 1
    actPtLife[stIdx[:,0]!=1] = 0
    ...
    if not(np.mod(frameIdx,5)) and (frameIdx != 300):
        newKp = kp1[newPtIdx]
