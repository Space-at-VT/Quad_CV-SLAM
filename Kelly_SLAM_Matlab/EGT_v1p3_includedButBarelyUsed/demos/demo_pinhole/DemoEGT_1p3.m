...
%% FIRST (Desired) CAMERA
  % Camera plot
  Kd=[700 0 640; 0 700 480; 0    0   1];
  Rd=rotoy(0)*rotox(0)*rotoz(0);
  td=[0,0,0]';
  Hd=f_Rt2H(Rd,td);
  f_3Dframe(Hd,'g',0.6,'_{d}');
  f_3Dcamera(Hd,'g',0.15); 
...
%% SECOND (Actual) CAMERA
  ...
  Ra=rotoy(pi/6)*rotox(0)*rotoz(0); 
  ta=[-.7,-0.7,0]';
  Ha=f_Rt2H(Ra,ta);
  f_3Dframe(Ha,'r',0.6,'_{c}'); 
  f_3Dcamera(Ha,'r',0.15);
  [ua,va]=f_perspproj(P,Ha,Ka,1); % ua is a 2xN matrix, with N = "columns of P"
  view(45,34);
  % Imaging model
...
% Epipoles and FUndamental Matrix Computation 
  ...
  sigma=.4;
  Ua=[ua+sigma*randn(1,length(ua(1,:)));va+sigma*randn(1,length(ua(1,:)))];
  Ud=[ud+sigma*randn(1,length(ua(1,:)));vd+sigma*randn(1,length(ua(1,:)))];

  Fstim4=f_Festim(Ua,Ud,4); % Robust M-estimator by Torr
  [la,ld]=f_epipline(Ua,Ud,Fstim4,1,3,2,'r');
  figure(4);

  display('[EGT message]: Ready to compare estimation methods over 30 iteration..press a key and wait for computation')
  pause
...
%% Epipolar Geometry Estimation with EGT
  tic
  Fstim1=f_Festim(Ua,Ud,1); % Linear estimation with no det(F)=0
  t1(j)=toc;
  tic;
  Fstim2=f_Festim(Ua,Ud,2); % Normalized 8-point algorithm
  t2(j)=toc;
  tic;
  Fstim3=f_Festim(Ua,Ud,3); % Minimization of Sampson Distance
  t3(j)=toc;
  tic;
  Fstim4=f_Festim(Ua,Ud,4); % Robust M-estimator by Torr
  t4(j)=toc;
