    ...
    if mode == "preinit":
        if not("hullBox" in locals()):
            ...
            if not("orb" in locals()):
                ...
                oldFrame = visfxns.GrayBlur(oldFrame,5,5,3)
            ...
            for ii in range(loopDummy,loopDummy+30):
                ...
                if not(success):
                    break
                frameIdx += 1
                newFrame = visfxns.GrayBlur(newFrameColor,5,5,3)
                oldFrame,kp0,des0,p0,actPtLife,gavg,avgMotion = bigfunctions.TrackMatch(
                oldFrame,newFrame,firstFrame,frameIdx,rFrames,roiVec,kp0,des0,p0,
                                                   actPtLife,
                                                   orb,bf,
                                                   "motion",1)
