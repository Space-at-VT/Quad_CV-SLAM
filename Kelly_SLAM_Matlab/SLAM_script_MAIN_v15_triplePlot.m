...
%% --------------------------------------------INSTRUCTIONS----------------------------------------------
% DEPENDENCIES:
% -OpenCV: Install version 2.4.10 or similar from opencv.org. Follow recommended instructions, main folder in the C:/ directory.
%
% -mexopencv: Installation instructions are online. Your MATLAB needs to be able to call functins from 
% OpenCV. Test after the install that 'help cv' works (this command will list all the possible function calls). OpenCV functions are called like 'cv.warpAffine'. 
% Type help cv.functionName for the implementation instructions. The trickiest part of the install for me was adding the right variables to the system path.
% Make sure to read the instructions for this step carefully. The system path variables are found by right clicking on 'my computer', selecting 'properties', on
% the left pane select 'advanced system settings', then on the bottom right of the new window select 'environment variables'. This is where the system path is
% edited. Mexopencv does not include any of the CV functions in opencv, it is only a matlab wrapper that enables calling the opencv functions inside matlab.
% When opencv 3.0 is released, a matlab wrapper will be included as an option similar to the way opencv functions are commonly called from python.
%
% -sba: Pretty easy to install. Using version sba-1.6. Also includes a MEX file interface for the external function. The link is
% http://www.ics.forth.gr/~lourakis/sba/. Includes installation instructions, the MEX file needs to be compiled similar to the following code. Verify it is
% installed with 'help sba'. Additionally confirm operation through the demo files included using the matlab mex interface. Running through matlab is 
% slower than a C++ implementation. Additional iterations or application through a larger scope of bundle adjustment may be possible to increase accuracy 
% through a C++ implementation without drastically affecting run time.
% 
...
% %     else
% %       eval([ 'mex -largeArrayDims LINKFLAGS="$LINKFLAGS /NODEFAULTLIB:libcmt.lib" ' ...
% %         library_flags '-lclapack_nowrap -lf2c -lBLAS_nowrap ' sba_files ]); 
% %     end
% % end
%
% -MATLAB Computer Vision Systems Toolbox. Verify it is installed by typing 'ver' at the command prompt. Possible to replace these functions (mainly the FAST
% corner detector and the KLT tracker (vision.PointTracker) with opencv functions. The KLT tracker might be difficult to replicate directly, but it is possible
% using calcOptFlowPyrLK in opencv. They are theoretically the same, a pyramidal implementation of the Lucas-Kanade method.
% 
% 
% TESTING OPERATION WITH TRAINING VIDEOS:
% -Uncomment one line in the next section below that starts with 'vid=...'. These are linked to different videos used for development of the algorithm in the
% 'Data' directory. Notice some assign particular parameters. Some parameters work better than others for particular videos.
...
% vid = VideoReader('high_bright_backAndForth_2015-01-23-114514-0000.avi'); cropVid = 0; cal='ptgrey'; manualBoundingBox = [330 245 839-330 502-245]; subRegionCount = 5; countThreshold = 19; startFrame = 40; endFrame = 305; hammDistThresh = .65;
% vid = VideoReader('high_bright_2015-01-23-112011-0000.avi'); cropVid = 0; cal='ptgrey'; manualBoundingBox = [345 268 770-345 502-268]; subRegionCount = 5; countThreshold = 19; startFrame = 40;  initPixDist = .5; hammDistThresh = .65; endFrame = 400+startFrame;
% vid = VideoReader('high_bright_2015-01-23-111938-0000.avi'); cropVid = 0; cal='ptgrey'; manualBoundingBox = [345 268 770-345 502-268]; startFrame = 100;subRegionCount = 5; countThreshold = 19; initPixDist = 1; endFrame = 400+startFrame;
...
% vid = VideoReader('orbital_express_stk.avi'); cropVid = 0; cal='est'; manualBoundingBox = [1 1 623 391]; subRegionCount = 3; countThreshold = 9; initPixDist = 2;
% vid = VideoReader('orbital_express_clip_1.avi'); cropVid = 0; cal='est'; countThreshold = 19; subRegionCount = 9; manualBoundingBox = [1 1 320 317];

% HOMER data
% vid = VideoReader('homer_clip.avi'); cropVid = 1; cal = 'est'; crop = [100 640 460 1050]; subRegionCount = 3; countThreshold = 19; startFrame=10; initPixDist = .25; hammDistThresh = .65; maxBdErr = .01; %countThreshold = 9; hammDistThresh = .5; maxBdErr = .05; subRegionCount = 5;

% PRISMA data
%  vid = VideoReader('prisma_entire_clip.avi'); cropVid = 0; cal = 'est'; hammDistThresh = .75; startFrame = 50; maxBdErr = .05; endFrame = 450; 

% Some developmental videos - results may vary!!
...
bestSubRegionKeep = 100; % Limit to the number of points re-detected throughout motion in sub-regions
...
plotSubRectangles = 0;
plotMotionHulls = 1;
saveVideo = 0; saveSLAM = 1; doSLAM = 1; % Change saveVideo to 0 or 1 if you want a video to be saved
if saveVideo == 1;
    obj=VideoWriter('homer_1.avi'); % The video output filename
    obj.FrameRate=15;
    open(obj);
end
...
%% Add in a binary feature extractor and matching routine to increase robustness of point tracks through verification tests
...
% This is frivolous use of space. A potential improvement is not keeping track of ALL points.
...
pointTracker = vision.PointTracker('MaxBidirectionalError', maxBdErr, 'NumPyramidLevels',3); % MATLAB's implementation of the KLT tracker
...
H=figure('units','normalized','outerposition',[0 .04 1 .96],'color','black'); % Maximizes the figure window
% figure('color','black');
...
h1 = subplot_tight(2, 2, [1 2],0.01);
h1 = imshow(plotFrame);
h2 = subplot_tight(2,2,3,0.01);
grid on;
% h3 = subplot_tight(2,2,4,0.01);

%% Extra initializations to be used
mode = 'preinit'; % mode defines the current point of the process in SLAM
fpsLog = []; totalFpsLog = []; fps = []; 
...
%         plotFrame = insertMarker(plotFrame, firstLoc(:,2:3), 'o', 'Color', 'red'); 
        ...
        %% Automate initialization here based on the pixel motion and initPixDiss
        ...
        % Display the 2D track points that exist in the 3D point cloud
        ...
        % Commented out, but this will display all KLT track points if desired
    ...
    if strcmp(mode,'pnp_poseOnly')
        ...
        [R, T] = cv.solvePnPRansac(pointCloud(2:4,pcIndex)',visiblePoints(has3dIndex,2:3),K1,[],'ReprojectionError',8,'IterationsCount',100);
        ...
        if lastT/initialT > .5 && pnpCtr > 5 || sum(pcIndex) < 12 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            % Some display information used for debugging. Uncomment if desired:
...
%             disp(dispStr);
    ...
    if strcmp(mode,'init')
        ...
        % Returns P, the collection of camera pose estimates (combination of camera calibration, rotation matrix, and translation vector from the center of the
        % world coordinate frame. Theory and notation available in thesis document.)
        [P, pointCloud] = initialization(firstLoc, currLoc, K1, K2);
        % The intiial implementation only performed SBA on the first 2 poses and structure estimates generated from intialization. Commented below. Currently,
        % SBA is performed on the first 3 pose estimates to increase the accuracy and # of observations.
...
%         end
        ...
        currentPose = P{end}; % Store the current pose for various uses.
    ...
    elseif strcmp(mode,'pnp_triangulate') % PnP with triangulation
        ...
        currentPose = P{end}; % Update the current pose value
...
%     break;
    ...
    if exist('pointCloud','var') % If there is a point cloud to draw
        ...
        % A janky implementation that rejects bad current pose estimates. May be a better idea to replace with a Kalman filter. For display. The current pose is
        % calculated with a reduced iteration of RANSAC on the pnp problem, which reduces confidence. Sometimes there may be bad estimates, which this seeks to
        % reject and display the prevoius good pose estimate.
        plotPose = currentPose;
        camCenter = -inv(plotPose(:,1:3))*plotPose(:,4);
        camFrame = invK*plotPose(:,1:3);
        camR = cv.Rodrigues([0 0 pi/2]) * camFrame;
        camRtoWorld = camR';
        up = camRtoWorld(:,1)';
        
        if P_index(end) == frameCtr
            prevCenter = camCenter;
            prevPose = currentPose;
        end
        
        magQnew = quaternion(plotPose); magQnew = magQnew(1);
        magQprev = quaternion(prevPose); magQprev = magQprev(1);

        distance = pdist2(camCenter',prevCenter');
        
        if distance > initialT
            plotPose = prevPose;
        else 
            prevCenter = camCenter;
            prevPose = currentPose;
