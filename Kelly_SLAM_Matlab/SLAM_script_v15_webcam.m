% v15_webcam:
% -Do it live from a USB webcam

%% Clear workspace, add paths
close all;
clear all;
clc; 
addpath(genpath('./'));
addpath(genpath('C:\Users\Daniel\Desktop\Scott\Data')); % data folder
...
if ~exist('initPixDist'), initPixDist = 2; end
if ~exist('countThreshold'), countThreshold = 9; end
if ~exist('subRegionCount'), subRegionCount = 5; end
...
%% Load webcam calibration
load('webcamCalibrationSession.mat');
cameraParams = calibrationSession.CameraParameters;
K1 = cameraParams.IntrinsicMatrix';
invK = inv(K1);
K2 = K1; % Since camera parameters are the same between views
...
cropVid = 0;
% Get first frame
motionFrame1 = rgb2gray(snapshot(cam));
height = size(motionFrame1,1); width = size(motionFrame1,2); diagLength = sqrt(height^2 + width^2);
manualBoundingBox = [1 1 width height];


%% Open video writer object if you want to save video
clear featureUpdateRate;
plotAll = 0;
plotSubRectangles = 0;
plotMotionHulls = 1;
saveVideo = 0; saveSLAM = 0; doSLAM = 1;
if saveVideo == 1;
    obj=VideoWriter('webcam_live_slam.avi');
    obj.FrameRate=15;
    open(obj);
end
...
motionFrame2 = rgb2gray(snapshot(cam));
...
% Add a global point number to tag along with each track point
...
%% Set up initial display figure

figure('units','normalized','outerposition',[0 .2 1 .85]); % Maximizes the figure window
...
fpsLog = []; fps = []; mode = 'preinit';
pnpCtr = 0; badSbaCtr = 0; currPnpRate = 5;


% for i = 1+stepSize:stepSize:300
while(1)
    tic
    ...
    currFrame = rgb2gray(snapshot(cam)); % Get the new frame    
